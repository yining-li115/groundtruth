// @ts-nocheck — vendored port of React Bits' LiquidEther (JS variant, MIT). A GPU fluid
// simulation (Stam stable-fluids: advection / viscosity / pressure-Poisson) whose velocity field
...
type LiquidEtherProps = {
  ...
  /** Returns the on-screen cursor in viewport px (or null). When set, drives the fluid instead
   *  of the built-in window mouse/touch listeners. */
  pointerSource?: (() => { x: number; y: number } | null) | null;
};
...
export function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = ["#5227FF", "#FF9FFC", "#B497CF"], // upstream placeholder — callers pass brand colors
  style = {},
  className = "",
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  // When set, returns the on-screen cursor position in viewport px (or null). Drives the fluid
  // instead of the built-in window listeners. Read via a ref so changing it never re-inits.
  pointerSource = null,
}: LiquidEtherProps) {
  ...
  useEffect(() => {
    ...
    const hasExternalPointer = !!pointerSourceRef.current;
    ...
    class MouseClass {
      ...
      init(container, bindListeners = true) {
        this.container = container;
        if (!bindListeners) return; // external pointer drives setCoords directly
...
`;
    ...
    class WebGLManager {
      constructor(props) {
        ...
        Mouse.init(props.$wrapper, !props.pointer);
      ...
      feedExternalPointer() {
        ...
        const p = this.props.pointer();
        if (!p) return;
        if (p.x !== this._lastPX || p.y !== this._lastPY) {
          this._lastPX = p.x;
          this._lastPY = p.y;
          Mouse.setCoords(p.x, p.y);
          this.lastUserInteraction = performance.now();
          if (this.autoDriver) this.autoDriver.forceStop();
        }
      }
      render() {
        this.feedExternalPointer();
    ...
    const webgl = new WebGLManager({
      ...
      autoResumeDelay,
      autoRampDuration,
      pointer: hasExternalPointer ? () => pointerSourceRef.current?.() ?? null : null
    });
