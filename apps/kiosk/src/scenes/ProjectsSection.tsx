...
/**
 * Projects — the group's papers/publications. A full-screen, left-aligned menu (adapted
 * from the Codrops RapidImageHoverMenuEffects demo 5: hover reveals number + venue tag),
 * and a click opens a ContentLayoutTransition-style detail (left: title/authors/abstract/
 * link; right: the paper's figures via swipe + dots). Data is content (content/papers.json,
 * CLAUDE.md rule 3). Dark via [data-theme="dark"] — a deliberate exception to the light-first
 * site (design-system §5); the rest of the kiosk stays light. `?open=<id>` deep-links a paper.
 */
export function ProjectsSection() {
  ...
  const selectedIndex = selected ? papers.findIndex((p) => p.id === selected.id) : -1;
  const goToPaper = (dir: -1 | 1) => {
    const next = papers[selectedIndex + dir];
          ...
          >
            ...
            <div className="pub-detail-right">
              ...
              <button
                ...
                disabled={selectedIndex >= papers.length - 1}
                onClick={() => goToPaper(1)}
              >
                <svg viewBox="0 0 24 28" aria-hidden="true">
                  <path d="M12 3v21" />
